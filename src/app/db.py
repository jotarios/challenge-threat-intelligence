...
class Campaign(Base):
    ...
    __table_args__ = (CheckConstraint("status IN ('active', 'dormant', 'completed')", name="ck_campaigns_status"),)
...
def create_engine(dsn: str) -> AsyncEngine:
    ...
    return create_async_engine(async_dsn, pool_size=10, max_overflow=0)
